...
// Props type export kar diya, ab TypeScript me error nahi aayega
...
// Shader & ShaderMaterial (unchanged)
...
const ShaderMaterial = ({ source, uniforms, maxFps=60 }: { source: string, uniforms: Uniforms, maxFps?: number }) => {
  ...
  const getUniforms = () => {
    ...
    for(const key in uniforms){
      const u: any = uniforms[key];
      switch(u.type){
        case "uniform1f": prepared[key]={value:u.value,type:"1f"}; break;
        case "uniform1fv": prepared[key]={value:u.value,type:"1fv"}; break;
        case "uniform3fv": prepared[key]={value:u.value.map((v:number[])=>new THREE.Vector3().fromArray(v)),type:"3fv"}; break;
        case "uniform2f": prepared[key]={value:new THREE.Vector2().fromArray(u.value),type:"2f"}; break;
        default: console.error(`Invalid uniform ${key}`); break;
      }
    }
